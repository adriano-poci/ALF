...
v1.4:   Plot a nicer corner plot. 24 October 2023
v1.5:   Prettify axis labels in corner plot. 11 November 2024
"""
...
class Alf(object):
    def __init__(self, outpath, mPath=None):
        ...
        self.labels = np.array(['chi2', 'velz', 'sigma', 'logage', 'zH', 'FeH',
            'a', 'C', 'N', 'Na', 'Mg', 'Si', 'K', 'Ca', 'Ti', 'V', 'Cr', 'Mn',
            'Co', 'Ni', 'Cu', 'Sr', 'Ba', 'Eu', 'Teff', 'IMF1', 'IMF2', 'logfy',
            ...
            'IMF3', 'IMF4', 'h3', 'h4', 'ML_v','ML_i','ML_k','MW_v',
            'MW_i','MW_k'])
        ...
        self.xH = results['Type', 'a', 'C', 'N', 'Na', 'Mg', 'Si', 'K', 'Ca',
            'Ti','V', 'Cr', 'Mn', 'Co', 'Ni', 'Cu', 'Sr','Ba', 'Eu']

        # Creating an empty dict
        # is filled in abundance_correct()
        self.xFe = dict()

        self.results = results[['Type']+self.labels.tolist()]
    ...
    def plot_corner(self, outname, params, color='k', save=True):
        ...
        mapping = {
            ...
            'FeH': 'Fe',
            'logfy': r'$\log(f_Y)$',
            'a': 'O', 'IMF1': r'$\alpha_1$', 'IMF2': r'$\alpha_2$',
            'IMF3': r'$m_{\rm cut}$', 'h3': r'$h_3$',
            'h4': r'$h_4$', 'ML_v': r'$M/L_V$', 'ML_i': r'$M/L_I$',
        }
        labels = np.array([mapping.get(label, label) for label in self.labels])
        params = [mapping.get(param, param) for param in params]
        ...
        try:
            ...
            fig = corner(self.mcmc[:,use],
                labels=labels[use], smooth=0.8, plot_contours=False,
                labelpad=1.25, max_n_ticks=2, plot_datapoints=False,
                plot_density=True, fig=fig, pcolor_kwargs=dict(cmap=moncmap))
            for ax in fig.get_axes():
                ax.set_xlabel(ax.get_xlabel(), rotation=15)
                ax.set_ylabel(ax.get_ylabel(), rotation=15)
            if len(params) > 2:
                axes = fig.get_axes()
                middle = len(axes) // 2
                middle_row = middle // (len(params) + 1)
                middle_col = middle % (len(params) + 1)
                target_row = max(0, middle_row - 2)
                target_col = min(len(params), middle_col + 6)
                target_idx = target_row * (len(params) + 1) + target_col
                if target_idx < len(axes):
                    ax = axes[target_idx]
                    ax.text(0.5, 0.5, r'$X={\rm [}X/H{\rm ]}$', ha='center',
